...
const PujaRequestAccepted = () => {
    const navigate = useNavigate();
    const dispatch = useDispatch();
    const { astroPujaAcceptedData: pujaRequestAcceptedData } = useSelector(state => state.astropujaReducer);
    console.log(pujaRequestAcceptedData);

    //* Order History DataTable Columns
    const pujaRequestColumns = [
        { name: 'S.No.', selector: row => pujaRequestAcceptedData.indexOf(row) + 1, style: { backGroundColor: "#000", paddingLeft: "20px" } },
    ...
    useEffect(() => {
        ...
        dispatch(AstropujaActions.getAstroPujaAccepted())
    }, []);

    return (
        <>
            <MainDatatable data={pujaRequestAcceptedData} columns={pujaRequestColumns} title={'Accepted Puja Requests '} />
        </ >
    );
}

export default PujaRequestAccepted;